...
export const tenantAnnouncementsApi = {
  list: async (type = "", page = 1, per_page = 10) =>
    unwrap(await api.get("/tenant/announcements", { params: { type, page, per_page } })),

  urgentActive: async () =>
    unwrap(await api.get("/tenant/announcements/urgent/active")),

  read: async (id: number) =>
    unwrap(await api.post(`/tenant/announcements/${id}/read`)),
