    ...
    <form method="POST" action="{{ route('admin.rooms.store') }}" class="space-y-4">
      ...
      <div>
        ...
        <input name="code" value="{{ old('code') }}"
               ...
               placeholder="เช่น A101" required />
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input name="floor" type="number" min="1" value="{{ old('floor', 1) }}"
                 class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500"
                 required />
    ...
    @error('room_type_id') <div class="mt-1 text-sm text-rose-600">{{ $message }}</div> @enderror
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input name="price_monthly" type="number" step="0.01" min="0" value="{{ old('price_monthly', 0) }}"
                 class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500"
                 required />
        ...
        <div>
          ...
          <select name="status"
            ...
            @foreach(['vacant' => 'ว่าง', 'occupied' => 'ไม่ว่าง', 'maintenance' => 'ซ่อมบำรุง'] as $k => $v)
              <option value="{{ $k }}" @selected(old('status','vacant')===$k)>{{ $v }}</option>
            @endforeach
          </select>
