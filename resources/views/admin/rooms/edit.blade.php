<x-admin-layout title="แก้ไขห้องพัก">
  <div class="max-w-xl rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
    <form id="frm" class="space-y-4">
      <div>
        ...
        <input id="code" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500" required />
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input id="floor" type="number" min="0" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500" required />
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input id="price_monthly" type="number" step="0.01" min="0" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500" required />
  ...
  <script>
    ...
    async function loadMeta(selectedId) {
    ...
    async function loadRoom() {
      const res = await window.api.get(`/admin/rooms/${ROOM_ID}`);
      const r = res.data.data;
      document.getElementById('code').value = r.code || '';
      document.getElementById('floor').value = r.floor || 0;
      document.getElementById('price_monthly').value = r.price_monthly || 0;
      document.getElementById('status').value = r.status || 'vacant';
      await loadMeta(r.room_type_id);
    }
    ...
    document.addEventListener('DOMContentLoaded', async () => {
      ...
      document.getElementById('frm').addEventListener('submit', async (e) => {
        ...
        try {
          await window.api.put(`/admin/rooms/${ROOM_ID}`, payload);
